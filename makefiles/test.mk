...
test.coverage: ## Generate test coverage
	docker-compose run --rm testserver bash -c "python -m pytest --cov-report term --cov-report html:coverage --cov-config setup.cfg --cov=app/src/ app/test/"

test.lint: ## Lint python files with flake8
	docker-compose run --rm server bash -c "python -m flake8 ./app/src ./app/test"
